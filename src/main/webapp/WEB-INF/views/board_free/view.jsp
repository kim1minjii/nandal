...
<body>
	<section class="container">
		...
		<br>
		<!-- 본글 -->
		<table class="table table-sm" style="width:80%;margin:0 auto;">
			<thead>
				<tr>
					<!-- 제목 -->
					<td>
			...
			<tbody>
					...
					<tr>
						<td>
							<div class="pt-4 pb-2" align="center">
								<img src="resources/board/free/${dto.upload}" width="80%" />
							</div>
						</td>
					</tr>
				</c:if>
				...
				<tr>
					...
					<td><c:if test="${!empty dto.upload}">
							<a href="fcontentdownload.do?num=${dto.num}">
		...
		<div style="width:80%;margin:0 auto;">
			<ol>
				<c:forEach items="${bList}" var="dto" varStatus="status">
					...
					<c:choose>
						<c:when test="${user.usercode==dto.writer || user.usergrade==2}">
							<form name="rfrm" id="rfrm" method="post"
		...
		<c:choose>
			<c:when test="${user.usergrade==1 || user.usergrade==2}">
				<div class="card" style="width:80%;margin:0 auto;">
					<form name="frm" method="post" action="frview.do" >
						<div class="card-body">
...
<script type="text/javascript">
	$(document).ready(function() {
		//본글에 달린 버튼 
		$("#list").click(function() {
			$('#frm').attr("action", "flist.do").submit();
		});

		$("#reply").click(function() {
			$('#frm').attr("action", "fwrite.do").submit();
		});

		$('#update').click(function() {
			$('#frm').attr("action", "fupdate.do").submit();
		});
		...
		$('#delete').click(function() {
			...
			$('#frm').attr("action", "fdelete.do").submit();
		});
		...
		$('.rupdate').click(function() {
			...
			$('#rfrm').attr("action", "frupdate.do").submit();
		});
		...
		$('div>ol form').on('click', this, function(){
			...
			$(this).attr("action", "frdelete.do").submit();
		})
	});
</script>
