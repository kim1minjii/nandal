...
<body>
	<div id="bodywrap" class="container">
		...
		<!-- 검색창 -->
		<form action="/myapp/flist.do" method="get">
			<div class="form-wrap">
				...
				<c:when test="${user.usergrade == 1 ||user.usergrade == 2 }">
					<div class="container">
						<div class="col text-end">
							<button class="btn btn-outline-secondary btn-sm" type="button" onclick="location.href='/myapp/fwrite.do'">글쓰기</button>
						</div>
					</div>
				</c:when>
			</c:choose>
         ...
         <div class="pagelist text-center"  >
          <ul class="pagination justify-content-center" >
            ...
            <c:if test="${pv.startPage>1}">
               <a href="flist.do?currentPage=${pv.startPage-pv.blockPage}" class="page-link text-dark text-decoration-none" aria-label="Next">&laquo;</a>
            </c:if>   
            <!-- 이전 출력 끝 -->

            <!-- 페이지 출력 시작 -->
            <c:forEach var="i" begin="${pv.startPage}" end="${pv.endPage}">
               <span> <c:url var="currPage" value="flist.do">
            ...
            <c:if test="${pv.endPage<pv.totalPage}">
               <a href="flist.do?currentPage=${pv.startPage+pv.blockPage}" class="page-link text-dark text-decoration-none" aria-label="Next">&raquo;</a>
            </c:if>
            <!-- 다음 출력 끝 -->
            </ul>
         </div>
