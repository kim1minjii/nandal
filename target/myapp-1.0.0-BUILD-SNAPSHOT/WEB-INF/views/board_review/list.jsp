					...
					<c:when test="${user.usergrade == 2 || user.usergrade == 1}">
						<div class="container">
							<div class="col text-end">
								<button class="btn btn-outline-secondary btn-sm" type="button" onclick="location.href='rwrite.do?contentid=${contentid}'">글쓰기</button>
							</div>
						</div>
					</c:when>
				</c:choose>
				...
				<c:when test="${empty param.searchWord}">
					...
					<c:choose>
						<c:when test="${user.usergrade == 2 || user.usergrade == 1}">
							<div class="container">
								<div class="col text-end">
									<button class="btn btn-light row justify-content-end" type="button" onclick="location.href='rwrite.do?contentid=${contentid}'">첫 리뷰 작성하기</button>
								</div>
							</div>
						</c:when>
					</c:choose>	
         ...
         <div class="pagelist text-center">
          <ul class="pagination justify-content-center" >
            ...
            <c:if test="${pv.startPage>1}">
               <a href="rlist.do?currentPage=${pv.startPage-pv.blockPage}"class="page-link text-dark text-decoration-none" aria-label="Next">&laquo;</a>
            </c:if>
            <!-- 이전 출력 끝 -->
            ...
            <c:if test="${pv.endPage<pv.totalPage}">
               <a href="rlist.do?currentPage=${pv.startPage+pv.blockPage}" class="page-link text-dark text-decoration-none" aria-label="Next">&raquo;</a>
            </c:if>
            <!-- 다음 출력 끝 -->
            </ul>
         </div>
